...
com.logicpartners.designerTools.pharmaCode = function () {
  ...
  this.object = function (x, y, width, height, barcodeTitle) {
    ...
    var canvasHolder = $("<canvas></canvas>")
      .prop("width", "100")
      .prop("height", "1");
    this.name = "Pharmacode " + self.counter++;
    this.text = barcodeTitle || "Pharmacode";
    this.x = x;
    this.y = y;
    //this.width = width;
    this.height = height;
    ...
    this.draw = function (context) {
      ...
      for (var i = 0; i < cwidth; i++) {
        if (cData.data[i * 4 + 3] == 255) {
          // Black (barcode = black or white)
